...
//            miroring?.updateBounds(matrixValues)
            ...
            if (doubleTapToZoom && doubleTapDetected) {
                ...
                if (matrixValues[Matrix.MSCALE_X] != startValues!![Matrix.MSCALE_X]) {
                    ...
                    val zoomMatrix = Matrix(matrix_tmp)
                ...
                if (event.actionMasked == MotionEvent.ACTION_DOWN ||
                    ...
                    if (allowTranslate(event)) {
                        ...
                        matrix_tmp.postTranslate(xdistance, ydistance)
                    }
                    if (allowZoom(event)) {
                        matrix_tmp.postScale(scaleBy, scaleBy, focusx, focusy)
                        currentScaleFactor =
        ...
        resetAnimator?.addUpdateListener(object : AnimatorUpdateListener {
            ...
            override fun onAnimationUpdate(animation: ValueAnimator) {
                ...
                values[Matrix.MTRANS_X] =
                    values[Matrix.MTRANS_X] + xtdiff * `val`
                values[Matrix.MTRANS_Y] =
                    values[Matrix.MTRANS_Y] + ytdiff * `val`
                values[Matrix.MSCALE_X] =
                    values[Matrix.MSCALE_X] + xsdiff * `val`
                values[Matrix.MSCALE_Y] =
                    values[Matrix.MSCALE_Y] + ysdiff * `val`
                activeMatrix.setValues(values)
                imageMatrix = activeMatrix
                miroring?.update(imageMatrix)
            }
        })
    ...
    private fun first_touch(event: MotionEvent) {
        if (!has_first) return
        has_first = false
        onTouchEvent(event)
    }
    ...
    private fun update(miror_matrix: Matrix) {
//        scope_ui.launch {
        imageMatrix = miror_matrix
//        }
        val values = FloatArray(9)
    ...
    private var miroring: ZoomageView? = null
